...
class _WebViewScreenState extends State<WebViewScreen>
  ...
  callback: (args) async {
    ...
    try {
      ...
      // Some APIs return response in body, some return directly
      final Map<String, dynamic> body =
          (data['body'] is Map<String, dynamic>)
              ? data['body'] as Map<String, dynamic>
              : data;

      // Extract token from multiple possible structures
      String? accessToken =
          body['token']?.toString() ??
          (body['data'] is Map
      ...
      debugPrint(
        '✅ Found Access Token: ${accessToken.substring(0, accessToken.length > 15 ? 15 : accessToken.length)}...',
      );

      await PrefsUtil.setAccessToken(accessToken);

      // Extract phone number from multiple possible structures
      String? phone;

      // Structure 1: user.phone
      if (body['user'] is Map) {
        phone = body['user']['phone']?.toString();
      }

      // Structure 2: data.user.phoneNumber
      if ((phone == null || phone.isEmpty) &&
          body['data'] is Map &&
          body['data']['user'] is Map) {
        phone = body['data']['user']['phoneNumber']?.toString();
      }

      // Structure 3: result.seller.phone (your current API)
      if ((phone == null || phone.isEmpty) &&
          body['result'] is Map &&
          body['result']['seller'] is Map) {
        phone = body['result']['seller']['phone']?.toString();
      }

      if (phone != null && phone.isNotEmpty) {
        debugPrint('📱 Found Phone Number: $phone');
        await PrefsUtil.setPhoneNumber(phone);
      } else {
